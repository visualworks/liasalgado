...
class Combined_Image_Text_Widget extends WP_Widget {
	...
	private $multiImageAlts = FALSE; //Enable multilangual image alts? (multilanguage only)
	...
	function __construct() 
	{
		...
		if ( function_exists('icl_get_default_language') && (icl_get_default_language()) )
		{
			...
			$this->multiImageAlts = get_option('citw_enable_img_alt', FALSE);
		}
	...
	public function register_settings_options()
	{
		...
		register_setting( 'citw-settings', 'citw_enable_img_alt');
	}
		 ...
		 ?>
		 	<div class="wrap">
				...
				<form method="post" action="options.php">
					...
					<tr valign="top">
					<th scope="row"><?php _e('Enable Multilingual Image Alts', 'citw'); ?></th>
					<td>
						<fieldset><legend class="screen-reader-text"><span><?php _e('enable multilingual image alts', 'citw'); ?></span></legend>
						<p><input type="checkbox" name="citw_enable_img_alt" value="1" <?php checked('1', get_option('citw_enable_img_alt', FALSE)); ?> /></p>
						</fieldset>
					</td>
					</tr>
					</table>
    ...
    **/
	...
	public function widget( $args, $instance ) 
	{
		if ( $this->isMultilingual )
		{
			$title_name = 'title_' .ICL_LANGUAGE_CODE;
		 	...
		 	if ( $this->urlSchema == 'auto' && isset($instance['widget_link']) && $instance['widget_link'] != '' ) $widgetLink = $this->add_language_to_url($instance['widget_link']);
		...
		else 
		{
			...
			$text_name = 'text';
			$image_name = 'image';
			$widgetLink = ( isset($instance['widget_link']) && ($instance['widget_link'] != '') )? $instance['widget_link'] : FALSE;
		}

		if ( $this->isMultilingual && $this->multiImageAlts )
		{
			$image_alt_name = 'image_alt_' . ICL_LANGUAGE_CODE;
			$imageAlt = ( isset($instance[$image_alt_name]) && ($instance[$image_alt_name] != '') )? $instance[$image_alt_name] : FALSE;
		}
		else $imageAlt = ( isset($instance['image_alt']) && ($instance['image_alt'] != '') )? $instance['image_alt'] : FALSE;

		$imageAlt = ($imageAlt)? 'alt="' . esc_html($imageAlt) . '"' : '';
		$widgetId = ( (isset($instance['widgetId'])) && (!empty($instance['widgetId'])) && ($instance['widgetId'] != '') )? $instance['widgetId']:$args['widget_id'];
		$widgetImg = ( (isset($instance[$image_name])) && (!empty($instance[$image_name])) )? '<img src="' . $instance[$image_name] . '" '. $imageAlt . '>':'';
		...
		$widgetClasses.= ' widget citw_widget';
		$html = '<section id="' . $widgetId . '" class="' . $widgetClasses . '">';
		if ( $title ) $html.= '<h3 class="widget-title">' . $title . '</h3>';
		
		if ( $widgetLink ) $html.= '<div><a href="' . $widgetLink . '" >' . $widgetImg . '<span class="citw_inner_widget_text">' . $text . '</span></a></div>';
		else $html.= '<div>' . $widgetImg . '<span class="citw_inner_widget_text">' . $text . '</span></div>';
	...
	public function form( $instance ) 
	{
		...
		if ( $this->isMultilingual )
		{
	    	...
	    	foreach ($active_languages as $lang):
				...
				if ( $this->multiImageAlts )
				{
					$image_alt_name = 'image_alt_' . $lang['code'];
					$image_alt = ( (isset( $instance[$image_alt_name])) && (!empty($instance[$image_alt_name])) )? $instance[$image_alt_name] : '';
				}
			?>
				...
				<?php if ( $this->multiImageAlts ): ?>
				<p>
		        <label for="<?php echo $this->get_field_id( $image_alt_name ); ?>"><?php echo $lang['display_name'] . ' Image Alt:'; ?></label> 
		        <input class="widefat" id="<?php echo $this->get_field_id( $image_alt_name ); ?>" name="<?php echo $this->get_field_name( $image_alt_name ); ?>" type="text" value="<?php echo esc_attr( $image_alt ); ?>">
		        </p>
				<?php endif;
			 	endforeach;
				...
				?>
				<p>
					<label for="<?php echo $this->get_field_id( 'image_alt' ); ?>"><?php _e( 'Image Alt:', 'citw' ); ?></label> 
					<input class="widefat" id="<?php echo $this->get_field_id( 'image_alt' ); ?>" name="<?php echo $this->get_field_name( 'image_alt' ); ?>" type="text" value="<?php echo esc_attr( $imageAlt ); ?>">
				</p>
				<?php endif;

				if ( $this->urlSchema != 'manual' ):
					// Generate widget link input
					$widgetLink = ((isset($instance['widget_link'])) && (!empty($instance['widget_link'])))? $instance['widget_link'] : '';
				?>
				<p>
					<label for="<?php echo $this->get_field_id( 'widget_link' ); ?>"><?php _e( 'Widget Link: (will dynamically change by language)', 'citw' ); ?></label> 
					<input class="widefat" id="<?php echo $this->get_field_id( 'widget_link' ); ?>" name="<?php echo $this->get_field_name( 'widget_link' ); ?>" type="text" value="<?php echo esc_attr( $widgetLink ); ?>">
				</p>	
			<?php endif;
		}
		else
		{
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'widget_link' ); ?>"><?php _e( 'Widget Link:', 'citw' ); ?></label> 
				<input class="widefat" id="<?php echo $this->get_field_id( 'widget_link' ); ?>" name="<?php echo $this->get_field_name( 'widget_link' ); ?>" type="text" value="<?php echo esc_attr( $widgetLink ); ?>">
			</p>	
			<?php
		}
	...
	public function update( $new_instance, $old_instance ) 
	{
		...
		if ( $this->multiImageAlts )
		{
			...
			foreach ($active_languages as $lang):
				$image_alt_name = 'image_alt_' . $lang['code'];
				$new_instance[$image_alt_name] = sanitize_text_field( $new_instance[$image_alt_name] );
			endforeach;		
		}
		else $new_instance['image_alt'] = sanitize_text_field( $new_instance['image_alt'] );
